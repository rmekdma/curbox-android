...
- **Current behavior:** `AppRuleBlocker.createGuardianApprovalIntent()`는 기존 `NEW_TASK` handoff에 명시적인 `SINGLE_TOP`을 함께 사용한다. `GuardianApprovalActivity`는 package identity와 denial list를 하나의 validated payload로 확인한 뒤 valid `onNewIntent()`에서만 intent와 selected rule을 갱신하고 기존 choice rows를 한 번 교체하므로 같은 activity instance와 one approval flow를 유지한다. package가 비어 있거나 denial list가 비어 있거나 entry가 null이거나 `ruleId`가 blank인 repeated payload는 `super.onNewIntent()`를 먼저 호출한 뒤 `setIntent()`와 Guardian state replacement 전에 거부되어 현재 화면, intent와 close package를 유지한다. malformed initial payload는 안전하게 finish한다. `ruleName`과 `reason`의 기존 표시 동작은 유지한다. grant와 skip flow는 시작 시점의 rule id를 authentication/write callback까지 보존한다. 기존 `onStop()` close notification과 receiver unregister cleanup도 유지된다.
...
- **Evidence:**
  ...
  - [Ticket 29 execution record](../../.scratch/app-rule-enforcement/issues/29-xiaomi-pad-android-16-oem-deep-sleep-final-validation.md): 2026-09-13 execution start captured connected device `iPlay50_mini_Pro` (Android 13 / SDK 33, build `TP1A.220624.014`, observed channel `unavailable`), confirmed model and Android major mismatch against required `Xiaomi Pad Pro 2025 12.7` (Android 15/16), stopped execution immediately without device substitution, and recorded objective OEM deep-sleep protocol proposal; actual test conditions and validation results remain pending target hardware and user approval.
...
- **Target refactor phase:** `Phase 0`에서 시나리오와 계측을 준비하고 `Phase 1`의 production adapter 검증에서 실행한다. Ticket 29 remains open pending target hardware availability and explicit user protocol approval.
...
- **Evidence:**
  ...
  - [ticket 28 drain budget evidence](../../.scratch/app-rule-enforcement/issues/28-drain-budget-evidence-decision.md#implementation-evidence--2026-09-13): production stop/drain/cancellation evidence documented; production preserves `RecoveryOnlyStop`; decision question presented to the user
...
- **Target refactor phase:** the ticket15/ticket18 implementation contracts are recorded complete. Ticket 27 p95 measurement and ticket 28 drain-budget decision evidence are recorded. Production remains on `RecoveryOnlyStop` until an explicit product threshold decision is made.
