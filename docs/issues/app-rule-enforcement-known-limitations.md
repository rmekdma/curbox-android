...
- **Current behavior:** `AppRuleBlocker.createGuardianApprovalIntent()`는 기존 `NEW_TASK` handoff에 명시적인 `SINGLE_TOP`을 함께 사용한다. `GuardianApprovalActivity`는 valid `onNewIntent()`에서 intent와 selected rule을 갱신하고 기존 choice rows를 한 번 교체하므로 같은 activity instance와 one approval flow를 유지한다. malformed 또는 empty repeated payload는 현재 화면을 지운 뒤 finish하지 않는다. 기존 `onStop()` close notification과 receiver unregister cleanup은 유지된다.
...
- **Evidence:**
  - [GuardianApprovalActivity.kt](../../app/src/main/java/neth/iecal/curbox/ui/activity/GuardianApprovalActivity.kt)의 `onNewIntent()`, `readDenials()`와 single-render path
...
- **Acceptance criteria:** focused Guardian lifecycle connected tests pass `3/3`; the repeated request has one activity instance, one approval choice row and updated denial content; close broadcast and background finish still pass; full JVM is `351/351`; and the full connected result classifies only T26's debug fixture as remaining. These results are iPlay50 deterministic evidence, not Xiaomi acceptance.
