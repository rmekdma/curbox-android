...
- [x] After approval, verify that the approved measurement path can observe the selected boundaries without changing decision behavior solely to obtain a number, then run only the approved protocol and record the observed p95, exact conditions, population, retained evidence, and known limitations in the canonical documentation.
- [x] Do not invent a pass or fail threshold, target timing, completion guarantee, or implementation response; if a threshold decision is needed, stop and ask the user for it.

### Observed measurement results

- **Run ID**: `T27-1789265859-2339`
- **Protocol**: `T27-P6`
- **Boundary**: Boundary **B** (fixed synthetic callback-to-decision-publication-entry)
- **Population**: $M1$ (20 warm-up attempts + 200 measured attempts = 100 allow + 100 deny)
- **Attempt Totals**:
  - Total attempts: 220
  - Warm-up attempts: 20 (valid: 20)
  - Measured attempts: 200 (valid: 200; valid allow: 100, valid deny: 100)
  - Exclusions: 0 (cap: 5)
  - Terminal totals: `VALID_SAMPLE`: 220
- **Observed Latencies (Boundary B, callback-to-decision-publication-entry)**:
  - Minimum: 12.323 ms
  - p50 (median): 28.110 ms
  - p90: 55.231 ms
  - **p95**: **63.040 ms**
  - p99: 83.827 ms
  - Maximum: 101.791 ms
  - Allow subset: p50 = 20.598 ms, p95 = 29.340 ms (N = 100)
  - Deny subset: p50 = 40.382 ms, p95 = 70.082 ms (N = 100)
- **Diagnostic Latencies (Boundary A, callback duration)**:
  - p50: 6.193 ms
  - p95: 10.134 ms
- **Retained Evidence**:
  - Staging directory on host: `.scratch/app-rule-enforcement/evidence/ticket27/T27-1789265859-2339/`
  - External manifest: `.scratch/app-rule-enforcement/evidence/ticket27/evidence-manifest.json`
  - On-device staging deletion verified (`deviceStagingDeleted: true`).
- **Known Limitations**:
  - Synthetic single-window test topology without live production accessibility window manager or multi-window transitions.
  - Pinned single device model `iPlay50_mini_Pro` (Android 13, API 33). Not representative of all Android OEMs or battery throttling states.
  - Reported purely as observational baseline evidence; no pass/fail threshold is asserted.
